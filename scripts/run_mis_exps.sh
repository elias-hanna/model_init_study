...
environments=(redundant_arm_no_walls_limited_angles)
...
#         echo "Processing following folder"; pwd
#         python ~/Documents/thesis/dev/model_init_exps/model_init_study/scripts/plot_mean_over_all.py --init-methods ${methods[*]} --init-episodes ${episodes[*]} --environment $env --dump-path . --pred-err-plot-upper-lim ${pred_error_plot_upper_limits[$cpt]} --disagr-plot-upper-lim ${disagr_plot_upper_limits[$cpt]}
#         cd ..
#     done
#     cpt=$((cpt+1))
#     echo "finished plotting pred errors for $env\n\n"
...
for env in "${environments[@]}"; do
    for sup_arg in "${sup_args[@]}"; do
        cd ${env}_${sup_arg}_results
        echo "Processing following folder"; pwd
        python ~/Documents/thesis/dev/model_init_exps/model_init_study/scripts/plot_hist_over_all_single.py --init-methods ${methods[*]} --init-episode ${episodes[*]} --environment $env --dump-path .
        cd ..
        cpt=$((cpt+1))
        echo "finished plotting histograms for $env"
    done
